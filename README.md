...
- **Coding-agent integrations** for IBM Bob, Codex, Claude Code, OpenCode, Cursor, Kiro, and Pi, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside IBM Bob, Codex, Claude Code, OpenCode, Cursor, Kiro, or Pi, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
The integration CLI supports **IBM Bob**, **Codex**, **Claude Code**, **OpenCode**, **Cursor**, and **Kiro**. All install at user level by default, so one installation works from any Git repository. Project paths are resolved to their Git repository root. User-level OpenCode integrations live under `~/.config/opencode`, IBM Bob uses `~/.agents/skills` and `~/.bob/mcp.json`, and Kiro uses `~/.kiro/skills` and `~/.kiro/settings/mcp.json`.

Pi uses its native package system. Install the published OpenWiki package with:

```sh
pi install npm:openwiki
```

The package includes the OpenWiki Pi extension and canonical skill. The extension starts the package-local `dist/cli/cli.js` through the current Node executable and keeps one MCP server process for each Pi session. Do not use `openwiki integrations install pi`.

Restart the coding agent after installation, open the repository, and ask:
